...
import { fetchCategoryThunk, fetchProducts, fetchSearchThunk } from '../src/store'
...
class Home extends Component {
							...
							<div className="panel panel-default category-products">
								<ul className="category-list">
									<li
										href="#"
										className={
											this.state.category === 'all' ? 'active' : undefined
										}
										onClick={() => this.handleSetProducts()}
										key={0}
									>
										ALL PRODUCTS
									</li>
								...
								<div className="search_box">
									<div className="input-group">
										<input
											...
											placeholder="Search our amazing smart home products"
										/>
										<span className="input-group-btn">
											<button
												className="btn btn-default"
												type="button"
												onClick={() => this.handleSetSearch()}
											>
												Search
											</button>
										</span>
	...
	handleSetSearch() {
		if (this.state.term) {
			this.setState(
				{ search: true, category: '', searchTerm: this.state.term },
